...
- The two hosts' copies of `TemplateElementSourceSupportTrait` are mirrored, not identical: `sourceInputProperties()` exists only on WordPress. Shared trait code must stand on the trait alone, and any tool schema added there needs a Joomla assertion in `docker/test-mcp-schema.php` — unit tests that only exercise behaviour will not catch a missing method in `getInputSchema()`.
- `template/element-source-set` takes `leaves` for several bindings under one `if_match`: fail-closed on the whole set, `set` values are deltas, and the preview lists every bound node with a state so a forgotten leaf is visible. Prefer it over repeating the single-path form. There is no `rewrites`; no real case has needed one.
