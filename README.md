...
The latest published release is [`0.10.1`](https://github.com/velisnolis/MirasAI/releases/tag/v0.10.1). See the [0.10.1 Style fixes](docs/releases/0.10.1.md). See the [0.10.0 migration notes](docs/releases/0.10.0.md) before updating translation callers. MirasAI includes:
...
| Automatic updates | XML feed serves published `0.9.0` until this tag exists | JSON feed serves published `0.9.0` until this tag exists | No feed; install the release tarball |
...
```text
packages/mirasai-wp/dist/mirasai-wp-0.10.0.zip
```
